...
async function init(){try{apps=await (await fetch('../data/apps.json')).json()}catch(e){apps=[]}render()}
input?.addEventListener('input',render);clear?.addEventListener('click',()=>{input.value='';input.focus();render()});chips.forEach(c=>c.addEventListener('click',()=>{chips.forEach(x=>x.classList.remove('active'));c.classList.add('active');category=c.dataset.category;render()}));init();